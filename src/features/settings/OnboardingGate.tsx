...
// Backend enforcement: session_manager (koe-e3m) reads `onboarding_completed`
// and `cost_tracker::can_start_session()` before allowing a session to start.
// This is a deliberate seam — koe-200 owns the UI gate, koe-e3m owns the
// session-start block.
//
...
export function OnboardingGate({ children }: OnboardingGateProps) {
  ...
  if (!loaded) {
    return (
      <div
        className="koe-onboarding-gate-loading"
        aria-label="読み込み中"
        role="status"
        aria-live="polite"
      >
  ...
  if (loadError) {
    return (
      <div className="koe-onboarding-gate-error">
  ...
  return (
    <div className="koe-onboarding-wizard">
      <h1 className="koe-onboarding-heading">koe へようこそ</h1>
      ...
      {budgetChoice === null ? (
        ...
        <div className="koe-onboarding-apikey-step">
          ...
          {finalizeError && (
            <p role="alert" className="koe-onboarding-error">
              {finalizeError}
            </p>
          )}
